netVisual_bubble2 <- function(object.list, group =NULL, sampleMetadata = NULL, group.col = NULL, sources.use = NULL, targets.use = NULL, signaling = NULL, pairLR.use = NULL, sort.by.source = FALSE, sort.by.target = FALSE, sort.by.source.priority = TRUE, color.heatmap = c("Spectral","viridis"), n.colors = 10, direction = -1, thresh = 0.05,
  ...
  if (is.list(object.list[[1]]@net[[1]])) {
  ...
  if (is.null(comparison)) {
    cells.level <- levels(object.list[[1]]@idents)
    ...
    if (!is.null(group)) {
      if (is.null(sampleMetadata)) {
        stop("If group, need sample metadata.")
      }
      object.list <- object.list[rownames(sampleMetadata)[sampleMetadata[group.col] == group]]
      df.net.list <- lapply(object.list, subsetCommunication, slot.name = "net",
                                                             sources.use = NULL, targets.use = NULL,
                                                             signaling = NULL,
                                                             pairLR.use = NULL,
                                                             thresh = 0.05)
      for (i in 1:length(df.net.list)) {
        df.net.list[[i]]$sample_id <- names(object.list)[i]
      }
      
      df.net <- do.call("rbind", df.net.list)
    ...
    source.target <- paste(rep(NULL, each = length(NULL)), NULL, sep = " -> ")
